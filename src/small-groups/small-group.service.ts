...
export class SmallGroupService {
    ...
    async create(createSmallGroupDto: CreateSmallGroupDto, user: any) {
        ...
        if (createSmallGroupDto.leaders && createSmallGroupDto.leaders.length > 0) {
            createSmallGroupDto.leaders = await this.userService.getUserIdsByDocuments(
                createSmallGroupDto.leaders,
            );
        }

        if (createSmallGroupDto.participants && createSmallGroupDto.participants.length > 0) {
            createSmallGroupDto.participants = await this.userService.getUserIdsByDocuments(
    ...
    async findAll(user: any) {
        ...
        if (user.role === Role.LIDER) {
            ...
            query = { group: { $in: assignedGroupIds } };
        }
    ...
    async findById(id: string, user: any) {
        ...
        // smallGroup.group is populated, we need to extract its _id safely
        const groupId = (smallGroup.group as any)?._id?.toString() || smallGroup.group?.toString();
        ...
        if (user.role === Role.LIDER) {
            ...
            if (!assignedGroupIds.includes(groupId)) {
                throw new ForbiddenException('No tenés permisos para ver este grupo pequeño');
            }
        }
    ...
    async update(id: string, updateSmallGroupDto: UpdateSmallGroupDto, user: any) {
        const smallGroupToUpdate = await this.smallGroupModel.findById(id).exec();
        ...
        if (user.role === Role.LIDER) {
            ...
            if (!assignedGroupIds.includes(smallGroupToUpdate.group.toString())) {
                throw new ForbiddenException('No tenés permisos para actualizar este grupo pequeño');
            }
        }

        if (updateSmallGroupDto.leaders) {
            updateSmallGroupDto.leaders = await this.userService.getUserIdsByDocuments(
                updateSmallGroupDto.leaders,
            );
        }

        if (updateSmallGroupDto.participants) {
            updateSmallGroupDto.participants = await this.userService.getUserIdsByDocuments(
    ...
    async delete(id: string, user: any) {
        const smallGroupToDelete = await this.smallGroupModel.findById(id).exec();
        ...
        if (user.role === Role.LIDER) {
            ...
            if (!assignedGroupIds.includes(smallGroupToDelete.group.toString())) {
                throw new ForbiddenException('No tenés permisos para eliminar este grupo pequeño');
            }
        }
