...
export class GroupService {
    ...
    async create(createGroupDto: CreateGroupDto) {
        ...
        if (createGroupDto.managers && createGroupDto.managers.length > 0) {
            createGroupDto.managers = await this.authService.getLiderIdsByUsernames(
                createGroupDto.managers,
            );
        }

        if (createGroupDto.collaborators && createGroupDto.collaborators.length > 0) {
            createGroupDto.collaborators = await this.userService.getUserIdsByDocuments(
        ...
        if (createGroupDto.managers && createGroupDto.managers.length > 0) {
            await this.authService.syncGroupManagers(
    ...
    async update(id: string, updateGroupDto: UpdateGroupDto, user: any) {
        ...
        if (user.role === Role.LIDER) {
            ...
            if (!assignedGroupIds.includes(existingGroup._id.toString())) {
                throw new ForbiddenException('No tenés permisos para actualizar este grupo');
            }
        }

        if (updateGroupDto.managers) {
            updateGroupDto.managers = await this.authService.getLiderIdsByUsernames(
                updateGroupDto.managers,
            );
        }

        if (updateGroupDto.collaborators) {
            updateGroupDto.collaborators = await this.userService.getUserIdsByDocuments(
    ...
    async delete(id: string) {
        ...
        // Remove this group from all managers
        await this.authService.syncGroupManagers(group._id.toString(), []);
